...
// ====== CONFIG ======
const String kOpenAiApiKey = "YOUR_OPENAI_API_KEY"; // TODO: replace securely (env/config)
const String kOpenAiModel = "gpt-4o-mini"; // inexpensive, good for intent JSON
const String kDefaultCity = "Mangaluru"; // optional, helps GPT understand places
  ...
  Future<void> _onSend() async {
    ...
    if (_pendingReport != null) {
      if (lower == 'confirm') {
        ...
        _pendingReport = null;
        _botSay("✅ Report submitted. Thank you for helping make $kDefaultCity safer.");
        return;
      } else if (lower == 'cancel') {
        _pendingReport = null;
        _botSay("❎ Okay, I canceled that report. You can start a new one anytime.");
        return;
      }
    }
    ...
    try {
      ...
      if (intent == 'check_area_safety') {
        ...
        _botSay(reply.isEmpty ? 'I\'ll check recent reports for that area.' : reply);
        return;
      }
      ...
      _botSay(reply.isEmpty ? "I\'m here to help with safety, reporting, or SOS." : reply);
    } catch (e) {
...
''';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('SafeZone Chatbot'),
      ),
      body: SafeArea(
        child: Column(
          children: [
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final align = m.fromBot ? CrossAxisAlignment.start : CrossAxisAlignment.end;
                  final radius = BorderRadius.only(
                    topLeft: const Radius.circular(16),
                    topRight: const Radius.circular(16),
                    bottomLeft: m.fromBot ? const Radius.circular(4) : const Radius.circular(16),
                    bottomRight: m.fromBot ? const Radius.circular(16) : const Radius.circular(4),
                  );
                  return Column(
